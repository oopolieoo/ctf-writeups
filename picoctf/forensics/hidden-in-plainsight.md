...
In future CTF's like this one, I will look to explore the files metadata earlier in my analysis. From my past experience, it seems like CTF authors usually hide a clue in the metadata; especially for those categorized as "forensics." In addition, I will be more cautious abandoning a path in the future. When I first could not find the `steghide` password, I tried a different approach to find the flag. In reality, my focus should have been on finding the missing information for the `steghide` command. 
